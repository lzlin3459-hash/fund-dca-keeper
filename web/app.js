...
async function fetchGlobalData() {
  try {
    ...
    // 顶部预算 badge 动态化
    const budget = data.profile && data.profile.plannedMonthlyDca;
    if (document.getElementById('dcaStatusBadge') && budget) {
      document.getElementById('dcaStatusBadge').innerText = `📅 每月5号定投 ¥${budget}/月（按真实估值自动调整每只扣款）`;
    }
  } catch (err) {
...
function renderTopFundValuationBoard(portfolioFunds = [], indices = []) {
  ...
  gridContainer.innerHTML = portfolioFunds.map(fund => {
    const matchedIdx = indexMap[fund.benchmarkIndex] || { pePercentile: 50, pe: 0, name: '对标', monthlyChange: 0, lastDayNavStatus: '月末企稳', signalText: '🟡 估值合理 (1.0x定投)', multiplier: 1.0, mode: '默认' };
    const pePct = fund.pePercentile || matchedIdx.pePercentile || 50;
    const curPe = matchedIdx.pe || 0;
    ...
    // 直接采用后端真实估值引擎算出的信号，不再前端重算
    const directiveText = matchedIdx.signalText || '🟡 估值合理 (1.0x定投)';
    const mult = matchedIdx.multiplier !== undefined ? matchedIdx.multiplier : 1.0;
    let badgeClass = 'badge-info';
    if (mult >= 1.5) badgeClass = 'badge-success';
    else if (mult <= 0) badgeClass = 'badge-warning';
    else if (mult < 1) badgeClass = 'badge-gold';

    // 诚实区分数据口径：宽基=历史分位，科技行业=当前PE绝对值
    const isPeMode = (matchedIdx.mode || '').includes('PE');
    const valText = isPeMode
      ? `PE ${pePct} 倍 · 当前值`
      : `${pePct}% · 历史分位`;
    ...
    return `
      <div class="fund-card-item">
        ...
        <div class="fund-card-meta">
          <span>板块估值</span>
          <span class="fund-card-pe">${valText} (${matchedIdx.name || '对标'})</span>
        </div>
        <div class="fund-card-meta">
          <span>月末净值表现</span>
        ...
        <div class="fund-card-footer">
          ...
          <span style="font-size:0.85rem; color:#34d399; font-weight:800; font-family:var(--font-heading);">
            <i class="fa-solid fa-calendar-check" style="margin-right:0.2rem; color:#f59e0b;"></i> 本月定投 ¥${dcaAmt}
          </span>
        </div>
        ${Math.abs(mult - 1.0) > 0.01 ? `
        <div class="fund-card-meta" style="margin-top:0.3rem; padding-top:0.3rem; border-top:1px dashed rgba(255,255,255,0.08);">
          <span style="font-size:0.78rem; color:#94a3b8;">定投调整</span>
          <span style="font-size:0.78rem; font-weight:700; color:${mult < 1 ? '#fbbf24' : '#34d399'};">
            原 ¥${Number(fund.baseDca || 0)} → 现 ¥${Number(dcaAmt)} ${mult < 1 ? `（少投 ¥${Number(fund.baseDca || 0) - Number(dcaAmt)}）` : `（多投 ¥${Number(dcaAmt) - Number(fund.baseDca || 0)}）`}
          </span>
        </div>` : ''}
      </div>
    `;
...
function bindEvents() {
  ...
  // Fund Modal：管理持仓 / 调整定投金额
  const fundModal = document.getElementById('fundModal');
  const openFundModal = () => {
    const funds = (globalState && globalState.trackedFunds) ? globalState.trackedFunds : [];
    document.getElementById('fundModalList').innerHTML = funds.map(f => `
      <div class="fund-dca-item" style="margin-bottom:.5rem; align-items:center;">
        <div>
          <span class="fname">${f.code} ${f.name}</span>
          <span style="font-size:.72rem; color:#64748b; margin-left:.4rem;">${f.category || ''}</span>
        </div>
        <div style="display:flex; align-items:center; gap:.4rem;">
          <span style="font-size:.78rem; color:#94a3b8;">¥/月</span>
          <input type="number" id="baseDca_${f.code}" value="${f.baseMonthlyDca}" min="0" step="5"
            style="width:80px; padding:.3rem .5rem; background:#0f172a; border:1px solid #334155; border-radius:6px; color:#e2e8f0;">
        </div>
      </div>
    `).join('');
  ...
  document.getElementById('btnSaveFunds').addEventListener('click', async () => {
    const funds = (globalState && globalState.trackedFunds) ? globalState.trackedFunds : [];
    const updates = {};
    funds.forEach(f => {
      const el = document.getElementById('baseDca_' + f.code);
      if (el) updates[f.code] = Number(el.value);
    });
